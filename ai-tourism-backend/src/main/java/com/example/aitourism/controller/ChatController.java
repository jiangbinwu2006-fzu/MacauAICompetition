...
public class ChatController {
    ...
    /**
     * Agent回调接口
     * 接收Python Agent格式化后的结构化JSON数据并存储到数据库
     */
    @SaIgnore
    @PostMapping("/callback")
    public BaseResponse<Void> callback(@RequestBody ChatCallbackRequest request) {
        log.info("收到Agent回调，sessionId: {}, userId: {}", request.getSessionId(), request.getUserId());
        
        if (request.getSessionId() == null || request.getSessionId().trim().isEmpty()) {
            return BaseResponse.error(Constants.ERROR_CODE_BAD_REQUEST, "缺少请求参数 session_id");
        }
        
        if (request.getStructuredOutput() == null || request.getStructuredOutput().trim().isEmpty()) {
            return BaseResponse.error(Constants.ERROR_CODE_BAD_REQUEST, "缺少请求参数 structured_output");
        }
        
        try {
            // 调用服务层保存结构化数据
            boolean success = chatService.saveStructuredOutput(
                request.getSessionId(), 
                request.getStructuredOutput()
            );
            
            if (success) {
                log.info("结构化数据保存成功，sessionId: {}", request.getSessionId());
                return BaseResponse.success();
            } else {
                return BaseResponse.error(Constants.ERROR_CODE_SERVER_ERROR, "保存结构化数据失败");
            }
        } catch (Exception e) {
            log.error("保存结构化数据异常: {}", e.getMessage(), e);
            return BaseResponse.error(Constants.ERROR_CODE_SERVER_ERROR, "内部服务器出错: " + e.getMessage());
        }
    }
}